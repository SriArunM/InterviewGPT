...
def generate_question(resume_data, description_data):
    ...
    data = llm(
        [
            SystemMessage(
                content="You need to Generate 5 Question each time on reading the resume and job description summaries from Human"
            ),
            HumanMessage(content=resume_data),
            HumanMessage(content=description_data),
        ]
...
if "sessionMessages" not in st.session_state:
    ...
    st.session_state.sessionMessages.append(
        SystemMessage(
            content="You will be Given Resume and Job Description to Analyze and Generate Questions and after user answers then you need to give him feedback"
        )
    )


data_questions = {"questions": [], "answers": []}
...
def generate_feeback():
    ...
    data = llm(
        [
            SystemMessage(
                content="Analyze all the Questions & Answers and then provide feedback."
                + questions
            ),
